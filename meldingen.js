(() => {
  ...
  async function deleteHandledNotice(id){
    ...
    try{
      const {c,session:s}=await session();
      const role=s.user?.app_metadata?.role||'internal';
      if(role==='external_reporter')throw new Error('Externe melders mogen geen meldingen verwijderen.');
      ...
      // Foto's eerst uit Storage verwijderen.
      const {data:files,error:listError}=await c.storage.from(MELD_PHOTO_BUCKET).list(String(id),{limit:20});
      if(!listError && files?.length){
        const paths=files.filter(f=>f?.name&&!f.name.startsWith('.')).map(f=>`${id}/${f.name}`);
        if(paths.length){
          const {error:photoDeleteError}=await c.storage.from(MELD_PHOTO_BUCKET).remove(paths);
          if(photoDeleteError)console.warn('Foto verwijderen:',photoDeleteError);
        }
      }
