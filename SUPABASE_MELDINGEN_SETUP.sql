...
on public.vappie_meldingen for select to authenticated using (true);

drop policy if exists "vappie meldingen aanmaken" on public.vappie_meldingen;
create policy "vappie meldingen aanmaken"
on public.vappie_meldingen for insert to authenticated
with check (created_by=auth.uid());

drop policy if exists "vappie reads eigen lezen" on public.vappie_melding_reads;
create policy "vappie reads eigen lezen"
on public.vappie_melding_reads for select to authenticated
using (user_id=auth.uid());

drop policy if exists "vappie reads eigen aanmaken" on public.vappie_melding_reads;
create policy "vappie reads eigen aanmaken"
on public.vappie_melding_reads for insert to authenticated
with check (user_id=auth.uid());

drop policy if exists "vappie reads eigen bijwerken" on public.vappie_melding_reads;
create policy "vappie reads eigen bijwerken"
on public.vappie_melding_reads for update to authenticated
using (user_id=auth.uid())
with check (user_id=auth.uid());
